...
async fn test_signaling_flow_and_track_notification() {
    ...
    assert!(res_a.sdp_offer.contains("urn:ietf:params:rtp-hdrext:sdes:mid"));
    assert!(res_a.sdp_offer.contains("urn:ietf:params:rtp-hdrext:sdes:rtp-stream-id"));
}
#[tokio::test]
...
async fn test_subscribe_logic() {
    ...
    let api = create_webrtc_api();
    let pc_a = Arc::new(api.new_peer_connection(RTCConfiguration::default()).await.unwrap());
    let codec = RTCRtpCodecCapability { mime_type: "video/VP8".to_owned(), ..Default::default() };
    let broadcaster = Arc::new(TrackBroadcaster::new("video".to_string(), codec, pc_a.clone(), 111));
    
    sfu.tracks.insert(format!("{}:{}:{}:{}", room_id, user_a, stream_id, track_id), broadcaster);
    
    // New Peer B joins
    let pc_b = Arc::new(api.new_peer_connection(RTCConfiguration::default()).await.unwrap());
    let peer_b = Peer {
        pc: pc_b,
    ...
    sfu.subscribe_to_existing_tracks(&peer_b, "userB", &room_id).await;
    
    // Verify B has a mapping for A's stream
    assert!(peer_b.track_mapping.contains_key(&stream_id));
    assert_eq!(peer_b.track_mapping.get(&stream_id).unwrap().value(), &user_a);
}
