 ...
 * 
 * ALL logs are completely silent in production (process.env.NODE_ENV === 'production').
 * This ensures users never see debug information in the browser console.
 * 
 * Features:
 * - Namespaced loggers for different modules (WebRTC, WebSocket, Media, Room, UI)
 * - Log levels (debug, info, warn, error)
 * - Conditional logging based on environment and configuration
 * - Structured log output with timestamps and colored output
 * - Performance timing utilities (logger.time())
 * - Grouping and table utilities for complex data
 * 
 * @example
 * ```typescript
 * import { createLogger, configureLogger } from '@/lib/logger';
 * 
 * // Create a namespaced logger
 * const logger = createLogger('WebRTC');
 * 
 * // Log with different levels
 * logger.debug('Connection details', { peerId: '123', state: 'connected' });
 * logger.info('Connection established', { peerId: '123' });
 * logger.warn('Reconnection attempted', { attempts: 3 });
 ...
 * 
 * // Group related logs
 * const group = logger.group('Processing participants');
 * logger.info('Added participant 1');
 * logger.info('Added participant 2');
 * group.end();
 * 
 * // Configure logging globally
 * configureLogger({ 
 *   minLevel: 'info',  // Hide debug logs
 *   enabledNamespaces: ['WebRTC', 'Media']  // Only log from specific namespaces
 * });
 * ```
 */
